...
func (m *MockAdapter) GetTransactions(token, accountID, consentID, requestingBank string, from, to time.Time, limit int) ([]Transaction, error) {
    ...
    for d := from; d.Before(to); d = d.AddDate(0, 1, 0) {
        ...
        if salaryDate.After(from) && salaryDate.Before(to) {
            transactions = append(transactions, Transaction{
                ...
                CounterpartyName: "ООО Работодатель",
                Category:        "salary",
            })
        }
    }
    ...
    categories := []struct {
        ...
        {"groceries", "Супермаркет", 1000, 5000},
        {"transport", "Транспорт", 200, 1000},
        {"restaurants", "Ресторан", 500, 3000},
        {"utilities", "ЖКХ", 3000, 8000},
    }
    ...
    products := []Product{
        {
            ...
            Description:  "Классический срочный вклад",
        ...
        },
        {
            ...
            ProductType:  "deposit",
            ProductName:  "Вклад Накопительный",
